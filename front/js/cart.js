...
for (let produit in produitLocalStorage){
    ...
    // Récupération du prix en utilisant l'ID produit
    var productUnit="";
    fetch("http://localhost:3000/api/produits/" + idProduit)
    .then(response => response.json())
        .then(async function (resultatAPI) {
            productUnit = await resultatAPI;
            // Insertion du prix
            var createpPrice = document.createElement('p');
            createpPrice.textContent = produitUnit.price + "€";
            productItemContentTitlePrice.appendChild(createpPrice);
        })
    ...
    for (let k = 0; k < qttModif.length; k++){
        qttModif[k].addEventListener("change" , (event) => {
            ...
            if (qttModifValue <= 0 || qttModifValue >= 100) {
                alert("Vous devez renseigner un nombre positif compris entre 1 et 100");
            }
            else {
                //Selection de l'element à modifier en fonction de son id ET sa couleur
                produitLocalStorage.map((obj) => {
                    if (obj.idProduit == produitLocalStorage[k].idProduit && obj.couleurProduit == produitLocalStorage[k].couleurProduit) {
                        obj.quantiteProduit = parseInt(qttModifValue);
                    }
                });
